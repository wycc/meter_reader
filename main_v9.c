...
static const char *s_tcp_port = "7001";
...
double get_meter_value(Meter *meter) {
    uint8_t tab_rp_bits[20];
    if (strcmp(meter->type,"energy_meter") == 0) {
        return read_energy_meter(meter->address);
    } else if (strcmp(meter->type, "water_meter") == 0) {
        get_sensor_data(&meter->sensor_ctx, tab_rp_bits);
        double degree = deconde_hex_reading(tab_rp_bits);
        printf("converted degree:%f\n",degree);
        return degree;
    } else {
        return 0.0;
    }
    ...
    if (has_config && ((int)start.tv_sec+start.tv_usec/1000000.0) + DATA_PUBLISH_INTERVAL/1000000.0 < ((int)stop.tv_sec+stop.tv_usec/1000000.0)) {
      ...
      if (!http_lock.lock) {
        ...
        for (i=0; i<meter_len; i++) {
          ...
          double data = get_meter_value(meter);
